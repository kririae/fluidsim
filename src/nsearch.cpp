...
// PURE cpu implementation, weak computation
...
hvector<SPHParticle> &NSearch::get_data()
{
  return data;
}
...
void NSearch::build()
{
  neighbor_map = hvector<hvector<uint>>(n_points());
    ...
    for (int u = grid_index.x - 1; u <= grid_index.x + 1; ++u) {
      for (int v = grid_index.y - 1; v <= grid_index.y + 1; ++v) {
        for (int w = grid_index.z - 1; w <= grid_index.z + 1; ++w) {
          ...
          const hvector<int> &map_item = hash_map[_hash_index];
...
hvector<uint> &NSearch::neighbor_vec(uint index)
{
