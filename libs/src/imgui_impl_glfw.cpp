...
//  2021-08-17: *BREAKING CHANGE*: Now using glfwSetWindowFocusCallback() to
//  calling io.AddFocusEvent(). If you called ImGui_ImplGlfw_InitXXX() with
//  install_callbacks = false, you MUST install glfwSetWindowFocusCallback() and
//  forward it to the backend via ImGui_ImplGlfw_WindowFocusCallback().
//  2021-07-29: *BREAKING CHANGE*: Now using glfwSetCursorEnterCallback().
//  MousePos is correctly reported when the host platform window is hovered but
//  not focused. If you called ImGui_ImplGlfw_InitXXX() with install_callbacks =
//  false, you MUST install glfwSetWindowFocusCallback() callback and forward it
//  to the backend via ImGui_ImplGlfw_CursorEnterCallback(). 2021-06-29:
//  Reorganized backend to pull data from a single structure to facilitate usage
//  with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2020-01-17:
//  Inputs: Disable error callback while assigning mouse cursors because some
//  X11 setup don't have them and it generates errors. 2019-12-05: Inputs: Added
//  support for new mouse cursors added in GLFW 3.4+ (resizing cursors, not
//  allowed cursor). 2019-10-18: Misc: Previously installed user callbacks are
//  now restored on shutdown. 2019-07-21: Inputs: Added mapping for
//  ImGuiKey_KeyPadEnter. 2019-05-11: Inputs: Don't filter value from character
                                ...
                                GlfwClientApi client_api)
{
  ...
  if (install_callbacks) {
    ...
    bd->PrevUserCallbackScroll = glfwSetScrollCallback(
        window, ImGui_ImplGlfw_ScrollCallback);
    bd->PrevUserCallbackKey = glfwSetKeyCallback(window,
                                                 ImGui_ImplGlfw_KeyCallback);
    bd->PrevUserCallbackChar = glfwSetCharCallback(window,
                                                   ImGui_ImplGlfw_CharCallback);
    bd->PrevUserCallbackMonitor = glfwSetMonitorCallback(
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
  ...
  for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) {
    io.MouseDown[i] = bd->MouseJustPressed[i] ||
                      glfwGetMouseButton(bd->Window, i) != 0;
...
#endif
  GLFWwindow *mouse_window = (bd->MouseWindow == bd->Window || focused) ?
                                 bd->Window :
                                 NULL;
...
void ImGui_ImplGlfw_NewFrame()
{
  ...
  if (w > 0 && h > 0)
    io.DisplayFramebufferScale = ImVec2((float)display_w / w,
                                        (float)display_h / h);
